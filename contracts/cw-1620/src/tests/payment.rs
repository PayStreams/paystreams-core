...
mod rounding_tests {
    use cosmwasm_std::{Addr, Coin, Uint128};
    use crate::{tests::suite::SuiteBuilder, ContractError, curve_helpers};
    ...
    fn test_rounding_small_deposit_over_long_duration() {
        ...
        suite.create_stream(
            ...
            "ibc/something/axlusdc",
            suite.get_time_as_timestamp().seconds(),
            suite.get_time_as_timestamp().plus_seconds(31536000).seconds(), // 1 year in seconds
            &[Coin {
                denom: "ibc/something/axlusdc".to_string(),
        ...
        assert_eq!(claimable_amount_halfway, Uint128::from(0u128).u128(), "Should be 0 due to rounding down");
        ...
        assert_eq!(claimable_amount_end, Uint128::from(1u128).u128(), "Full amount should be claimable");
    }
...
fn test_rounding_large_deposit_short_duration() {
    ...
    let mut suite = SuiteBuilder::new().with_funds(
        &funder.to_string(),
        &[Coin {
            denom: "ibc/something/axlusdc".to_string(),
            amount: Uint128::from(1000000000u128),
    ...
    suite.create_stream(
        ...
        "ibc/something/axlusdc",
        suite.get_time_as_timestamp().seconds(),
        suite.get_time_as_timestamp().plus_seconds(short_duration).seconds(),
        &[Coin {
            denom: "ibc/something/axlusdc".to_string(),
    ...
    assert_eq!(claimable_amount, Uint128::from(expected_claimable_halfway).into(), "Claimable amount should be half of the deposit after half duration");
    ...
    // Full amount should be claimable at the end
    let claimable_amount_end = suite.query_stream_claimable_amount(1u64).unwrap();
    assert_eq!(claimable_amount_end, large_deposit.into(), "Full amount should be claimable at the end of the duration");
}
...
fn test_rounding_edge_case_for_one_second() {
    ...
    let claimable_amount = suite.query_stream_claimable_amount(1u64).unwrap();
    assert_eq!(claimable_amount, deposit_for_one_second.into(), "Claimable amount should match deposit after one second");
}
...
fn test_rounding_with_intermittent_claims() {
    ...
    suite.create_stream(
        ...
        "ibc/something/axlusdc",
        suite.get_time_as_timestamp().seconds(),
        suite.get_time_as_timestamp().plus_seconds(duration).seconds(),
        &[Coin {
            denom: "ibc/something/axlusdc".to_string(),
            amount: deposit,
        }],
    ...
    for &interval in &intervals {
        ...
        let claimable_amount = suite.query_stream_claimable_amount(1u64).unwrap();
        suite.withdraw_from_stream(recipient.clone(), Uint128::from(claimable_amount).u128(), "ibc/something/axlusdc", Some(1u64)).unwrap();
        ...
        assert_eq!(total_claimed, expected_total_claimed, "Total claimed should not exceed expected amount");
    }

    // At the end of the stream, the remaining balance should be zero
    suite.update_time(duration);
    let remaining_balance = suite.query_stream_by_index(1u64).unwrap().streams.pop().unwrap().remaining_balance;
    assert_eq!(remaining_balance, Uint128::zero(), "Remaining balance should be zero at the end of the stream");
}
    ...
    .build();
        ...
        for &month_duration in &month_durations {
            ...
            // Move the blockchain time forward
            suite.update_time(seconds_in_a_month*month_duration);

            // Query the claimable amount
            let claimable_amount = suite.query_stream_claimable_amount(1u64).unwrap();
            
            // Assert that the claimable amount is as expected
            println!("Claimable amount after {} months is {}", month_duration, claimable_amount);
            // assert_eq!(claimable_amount, expected_amount.u128(), "Claimable amount after {} months is not as expected", month_duration);
