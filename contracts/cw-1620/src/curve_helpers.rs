use cosmwasm_std::{Uint128, Response, Timestamp, Deps, StdResult, Env};
use wynd_utils::{Curve, CurveError};

use crate::{ContractError, state::{StreamType, payment_streams, PaymentStream}};
...
pub fn calc_rate_per_second(duration: u64, deposit: Uint128) -> Result<Uint128, ContractError> {
    if duration == 0 {
        ...
        let scaled_deposit = deposit.u128().checked_mul(scaling_factor)
            .ok_or(ContractError::DivisionByZero {})?;

        let rate_per_second = scaled_deposit.checked_div(duration as u128)
            .ok_or(ContractError::DivisionByZero {})?
...
pub fn avail_balance_of(stream: PaymentStream, env: Env) -> Result<Uint128, ContractError> {
    ...
    match stream.curve {
        ...
        None => {
            ...
            let mut rate_per_second:Uint128;
            if stream.rate_per_second.is_zero(){
